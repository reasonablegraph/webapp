		...
		<div class="col-md-12 result-header" aria-hidden="true" >
			@if( $display_mode == 'normal' )
						...
						<span class="results_exp">
								@if ($numBooksFound>0 || $numIssuesFound>0)
									({{trChoise('are contained',$numBooksFound + $numIssuesFound)}}
									@if ($numBooksFound>0){{$numBooksFound}} {{trChoise('manif books',$numBooksFound)}} @if($numIssuesFound>0){{tr('and')}}@endif @endif
									@if ($numIssuesFound>0){{$numIssuesFound}} {{trChoise('manif issues',$numIssuesFound)}}@endif )
								@endif
					...
					<div class="col-sm-12 sb_label result-filter">
						...
						@foreach ($facetes_names as $name)
							@if (Input::has($name))
								@if($name =='form_type' || $name =='form_type_all' || $name =='pub_record_type')
