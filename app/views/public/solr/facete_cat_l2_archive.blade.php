...
if (!empty($facete_params['access_level']) && $facete_params['access_level'] == 'login'){
		...
		<div id="side-{{ $facete_name}}_facet" class="collapse {{$class_side_div}}" aria-expanded="true">
				@foreach ($facete_lines as $value => $count)
							<?php
								...
								if( PUtil::explodeIdFacet(Input::get('cat_l1')) != $cat_arr['label'] && !empty(Input::get('cat_l1')) ){
									//continue; ##Fix neighborhood
								}
								$is_selected = false;
								$selected_class = '';
